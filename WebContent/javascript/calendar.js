...
{
  var x=document.forms["check"]["datum1"].value;
  if (x==null || x=="")
  {
      alert("Choose a Date from Calendar");
      return false;
  }
}
function checkEnterDate()
{
	  ...
	  else
	  {
		  ...
		  var cDay=parseInt(curDay);
		  var cMonth=parseInt(curMonth)+1;
		  var cYear=parseInt(curYear);
		  var dd=x.substring(0,2);
		  var mm=x.substring(3,5);
		  var yy=x.substring(6,10);
		  myDate=parseInt(dd);
		  myMonth=parseInt(mm);
		  myYear=parseInt(yy);
		  if(myYear>cYear)
		  {
			  alert("You Cannot Assign Reward For Future Date");
			  document.getElementById("datum1").value="";
		  }
		  else if(myYear==cYear)
		  {
			  if(myMonth>cMonth)
			  {
				  alert("You Cannot Assign Reward For Future Date");
			  ...
			  else if(myMonth==cMonth)
			  {
				  if(myDate>cDay){
					  ...
					  alert("Rewards Assignment Can Be Done only 7 Days Prior To Taday's Date ");
						 document.getElementById("datum1").value="";  
				  }
				  else 
				  {
					  var one_day=1000*60*60*24;
					  var date1=new Date(curDay,curMonth,curYear);
					  var date2=new Date(myDate,myMonth,myYear);
...
/*function date_diff()
{
	    ...
	    var curDate = new Date();
		  ...
		  var mm=x.substring(3,5);
		  var yy=x.substring(6,10);
        // var date1=new Date("curMonth/curDay/curYear");
         //var date2=new Date("mm/dd/yy");
         var oneDay=1000*60*60*24;
       // alert(Math.ceil((date1.getTime()-date2.getTime())/oneDay));
         date1 = yy+"/"+mm+"/"+dd;
         date2 = curYear+"/"+curMonth+"/"+curDay;

         firstDate = Date.parse(date1);
         secondDate= Date.parse(date2);

       
          alert(Math.ceil((secondDate.getTime()-firstDate.getTime())/oneDay));
		  if(dbd<=7)
		  {
			  alert("Rewards AssPrior To Taday's Date ");
				 document.getElementById("datum1").value="";
		  }
}
*/